...
class DatUsageSpec < MiniTest::Spec
  ...
  it 'parses metadata from io' do
    ...
    Dat.parse_metadata(io).must_equal( {
      :mime_version => '1.0',
      :mascot_version => '1.0',
  ...
  it 'does not reposition io' do
    ...
    io = StringIO.new str
    io.pos = 10
    Dat.parse_metadata(io)[:boundary].must_equal "gc0p4Jq0M2Yt08jU534c0p"
    io.pos.must_equal 10
  end



  def queries_same?(query1, query2)
    ((query1.data == query2.data) && (query1.index == query2.index) && (query1.section_name == query2.section_name))
  end

  #
  # describe Dat.each_query, Dat.query, and Dat.queries
  #
  ...
  it 'gives direct access to queries' do
    ...
    Dat.open(@file) do |dat|
      ...
      # queries
      as_array = dat.queries
      as_array.zip(qrs) do |quer1, quer2|
        assert queries_same?(quer1, quer2)
      end
    end
    assert inside_dat, "the block was read"
  end
